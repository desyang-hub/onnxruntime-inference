...
TensorBuffer YoloDetector::preprocess(const cv::Mat& img) {
    ...
    return preprocess(std::vector<cv::Mat>{img});
}


ModelOutput YoloDetector::infer(const TensorBuffer& tenbuf) {
    
    return infer();
}


std::vector<Detection> YoloDetector::postprocess(const ModelOutput& model_output) {
    const TensorBuffer& tenbuf = model_output.primary();
    return postprocess(tenbuf, 1).front();
}
...
std::vector<Detection> YoloDetector::detect(const cv::Mat& img) {
    LOG_TRACE("YoloDetector::detect enter | size={}x{} type={}", img.cols, img.rows, img.type());
    preprocess(std::vector<cv::Mat>{img});

    ModelOutput infer_out = infer();

    auto res = postprocess(infer_out.primary(), 1).front();
    LOG_TRACE("postprocess done | detections={}", res.size());
