...
/// @param imgs 图像集合
/// @return 
std::vector<cv::Mat> NAFNet::restoration(const std::vector<cv::Mat>& imgs) {
    preprocess(imgs);
    ModelOutput infer_out = infer();
    
    std::vector<cv::Mat> res = postprocess(infer_out.primary(), imgs.size());
    return res;
}

cv::Mat NAFNet::restoration(const cv::Mat& img) {
    return cv::Mat{};
}
...
/// @brief 图像预处理
/// @param imgs 图像集合
void NAFNet::preprocess(const std::vector<cv::Mat>& imgs) {
    // ====== 1. 预处理 ======
