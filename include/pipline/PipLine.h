...
private:
            ...
            if ((!success && !inputs_.empty()) || inputs_.size() >= context_.batch_size) {
                do
                {
                    ...
                    std::shared_ptr<CPUBuffer> h_input;
                    {
                        ...
                        if (!h_inputs_.empty()) {
                            h_input = std::move(h_inputs_.front());
                            h_inputs_.pop();
                        }
                        else {
                            break;
                    ...
                    std::shared_ptr<GPUBuffer> d_buf;
                    {
                        ...
                        if (!d_buffers_.empty()) {
                            d_buf = std::move(d_buffers_.front());
                            d_buffers_.pop();
                        }
                        else {
                            break;
                    ...
                    std::shared_ptr<CPUBuffer> h_output;
                    {
                        ...
                        if (!h_outputs_.empty()) {
                            h_output = std::move(h_outputs_.front());
                            h_outputs_.pop();
                        }
                        else {
                            break;
    ...
    public:
    PipLine(const YAML::Node& config, 
        ...
        worker_ = std::thread(std::bind(&PipLine::submit_worker, this));
    }

    ~PipLine() {
        {
            std::lock_guard<std::mutex> lock(h_input_mutex_);
            std::lock_guard<std::mutex> lock1(d_buffer_mutex_);
            std::lock_guard<std::mutex> lock2(h_output_mutex_);
            std::lock_guard<std::mutex> lock3(inputs_mutex_);

            is_close_ = true;
        }
        h_input_condition_.notify_all();
        d_buffer_condition_.notify_all();
        h_output_condition_.notify_all();
    ...
    //     pre_process_.enqueue(std::bind(&PipLine::preprocess_worker, this, input, pms));
